    ...
    | x :: xs -> (
      ...
      match state.problem.initial_grid.(i).(j) with
        | None -> (
            ...
            (state.boxes_taken.(3 * (i/3) + j/3).(x - 1) > 1) || wrong_cages) in

            Some ({current_grid = state.current_grid; problem = state.problem; rows_taken = state.rows_taken; cols_taken = state.cols_taken;
